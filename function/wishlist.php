...
<body>
  <header class="pb-3">
    ...
    <div class="heading">
      <div class="heading-top mb-5">
        <h1>Cart</h1>
        <p class="text-uppercase">Choose your product to check out! </p>
      </div>
    </nav>
    </div>
  </header>
  ...
  <section class="result container-fluid">
    ...
    //$sql = "SELECT * FROM `cart` JOIN product USING(p_id) WHERE b_id = '$user_id' ";
    $sql = "SELECT * FROM `cart_product` 
    JOIN `product` USING(p_id) 
    WHERE `cart_product`.`cart_id` IN (SELECT `cart_id` FROM `cart` WHERE `b_id` = '$user_id')
    GROUP BY `cart_product`.`p_id`";
...
$query_time = number_format($end_time - $start_time, 5);
    ...
    echo "<h6 class='ps-3 pt-3'> We found <strong>{$num_results}</strong> Results <strong>\"Your Cart\"</strong> in {$query_time} seconds:</h6>";
    echo "</a>";
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $product_id = $row["p_id"];
        $order_id = 1;
        echo "<form method='post' action='check.php' enctype='multipart/form-data'>";
        ...
        echo "<input type='number' name='$fruitname' value='1' min='1' style='width: 100px; margin-right: 10px;'>";
        echo "<input name='p_id' type='hidden' value='" . $product_id . "'>";
        echo "<input name='o_id' type='hidden' value='" . $order_id . "'>";
        echo "<input name='b_id' type='hidden' value='" . $user_id . "'>";
        echo "<input name='price' type='hidden' value='" . $price . "'>";
        echo "<input type='checkbox' name='hlhl[]' value='$fruitname' style='transform: scale(1.8); cursor: pointer;' />";
        echo "    </div>";
        echo "  </div>";
        echo "</article>";
      }
      echo "<button type='submit' name='checkout' class='btn bg-red text-light' style='display: block; font-size: 12px; padding: 10px 20px;'>CheckOut</button>"; 
      echo "</form>";
